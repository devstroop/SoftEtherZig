...
pub const FdbTable = struct {
    ...
    pub fn initCapacity(allocator: std.mem.Allocator, capacity: usize) !FdbTable {
        ...
        const order = try allocator.alloc(MacAddress, capacity);
    ...
    fn hash(mac: MacAddress) u64 {
        // FNV-1a over the 6 MAC bytes; spread via mixing so a burst of
        // same-vendor MACs (identical OUI) does not cluster.
    ...
    fn probeStart(mac: MacAddress, cap: usize) usize {
        return @intCast(hash(mac) % cap);
    }

    /// Find the slot index for `mac` if present. Linear probe bounded by
    /// the table size; returns null when absent.
    fn findSlot(self: *const FdbTable, mac: MacAddress) ?usize {
        var i = probeStart(mac, self.slots.len);
        for (0..self.slots.len) |_| {
            if (!self.occupied[i]) return null;
            if (std.mem.eql(u8, &self.slots[i].mac, &mac)) return i;
            i = (i + 1) % self.slots.len;
        }
        return null;
    }

    /// Look up the port a MAC was last seen on.
    pub fn lookup(self: *const FdbTable, mac: MacAddress) ?u16 {
        const idx = self.findSlot(mac) orelse return null;
    ...
    pub fn learn(self: *FdbTable, mac: MacAddress, port: u16, now: u32) FdbError!void {
        ...
        for (0..self.slots.len) |_| {
            if (!self.occupied[i]) {
                self.slots[i] = .{ .mac = mac, .port = port, .last_seen = now };
                self.occupied[i] = true;
                self.count += 1;
                self.appendOrder(mac);
                return;
            }
            i = (i + 1) % self.slots.len;
        }
    ...
    fn appendOrder(self: *FdbTable, mac: MacAddress) void {
        if (self.order_len < self.order.len) {
            self.order[self.order_len] = mac;
            self.order_len += 1;
        } else {
            // Ring full (count == capacity): overwrite the head.
            self.order[self.order_head] = mac;
            self.order_head = (self.order_head + 1) % self.order.len;
        }
    }

    fn touchOrder(self: *FdbTable, mac: MacAddress) void {
        ...
        while (i < self.order_len) : (i += 1) {
            if (std.mem.eql(u8, &self.order[i], &mac)) {
                found = i;
                break;
            }
        }
        ...
        if (self.order_len < self.order.len) {
            ...
            self.order[self.order_len - 1] = mac;
        } else {
    ...
    fn evictOldest(self: *FdbTable) void {
        ...
        const victim = if (self.order_len < self.order.len) self.order[0] else self.order[self.order_head];
        if (self.findSlot(victim)) |idx| {
    ...
    fn rebuildOrder(self: *FdbTable) void {
        ...
        for (self.occupied, 0..) |occ, i| {
            if (!occ) continue;
            self.order[out] = self.slots[i].mac;
            out += 1;
        }
        self.order_len = out;
        self.order_head = 0;
...
test "fdb learn + lookup + refresh" {
    ...
    try std.testing.expectEqual(@as(?u16, 1), t.lookup(makeMac(1, 2, 3, 4, 5, 6)));
...
test "fdb aging drops idle entries, keeps fresh" {
    ...
    try std.testing.expectEqual(@as(?u16, null), t.lookup(makeMac(1, 1, 1, 1, 1, 1)));
    try std.testing.expectEqual(@as(?u16, 2), t.lookup(makeMac(2, 2, 2, 2, 2, 2)));
    try std.testing.expectEqual(@as(usize, 1), t.len());

    // Exactly at the boundary (aging_s elapsed) is still fresh.
    try t.learn(makeMac(1, 1, 1, 1, 1, 1), 1, 350);
    _ = t.age(650);
    try std.testing.expectEqual(@as(?u16, 1), t.lookup(makeMac(1, 1, 1, 1, 1, 1)));
}
...
test "fdb overflow floods via TableFull after evicting oldest" {
    ...
    try std.testing.expectEqual(@as(usize, 3), t.len());
    try std.testing.expectEqual(@as(?u16, null), t.lookup(makeMac(1, 0, 0, 0, 0, 0)));
    try std.testing.expectEqual(@as(?u16, 2), t.lookup(makeMac(4, 0, 0, 0, 0, 0)));
}
...
test "fdb re-learn on move (port flip)" {
    ...
    try std.testing.expectEqual(@as(?u16, 3), t.lookup(makeMac(1, 2, 3, 4, 5, 6)));
    try std.testing.expectEqual(@as(usize, 1), t.len());
}