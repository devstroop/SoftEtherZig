...
UINT Count(COUNTER *c)
{
	...
	// Validate pointer looks reasonable (catch use-after-free)
	if ((uintptr_t)c < 0x1000 || ((uintptr_t)c & 0x7) != 0)
	{
		// Bad pointer - either NULL-ish or misaligned
		return 0;
	}
...
UINT Release(REF *ref)
{
	...
	// Validate pointer looks reasonable (catch use-after-free)
	if ((uintptr_t)ref < 0x1000 || ((uintptr_t)ref & 0x7) != 0)
	{
		// Bad REF pointer - either NULL-ish or misaligned
		return 0;
	}
	
	// Validate ref->c before accessing it
	if (ref->c == NULL || (uintptr_t)ref->c < 0x1000 || ((uintptr_t)ref->c & 0x7) != 0)
	{
		// Bad COUNTER pointer - don't try to use it
		return 0;
	}
...
UINT AddRef(REF *ref)
{
	...
	// Validate REF pointer itself looks reasonable (catch use-after-free)
	if ((uintptr_t)ref < 0x1000 || ((uintptr_t)ref & 0x7) != 0)
	{
		printf("[AddRef] ERROR: ref=%p is garbage! Likely use-after-free!\n", ref);
		fflush(stdout);
		return 0;
	}

	// CRITICAL SAFETY CHECK: Validate ref->c looks like a valid pointer
	// On 64-bit systems, valid heap pointers are typically > 0x100000
	if (ref->c == NULL || ((uintptr_t)ref->c < 0x100000))
	{
		printf("[AddRef] ERROR: ref=%p has invalid c=%p! Likely use-after-free or corruption!\n", ref, ref->c);
		printf("[AddRef] Backtrace would be helpful here.\n");
		fflush(stdout);
		// Return 0 to indicate failure rather than crashing
		return 0;
	}
