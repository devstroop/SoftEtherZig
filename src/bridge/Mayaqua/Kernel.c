...
void ThreadPoolProc(THREAD *t, void *param)
{
	...
	while (true)
	{
		...
		// Validate thread pointer before accessing it
		if (thread == NULL || (uintptr_t)thread < 0x1000 || ((uintptr_t)thread & 0x7) != 0)
		{
			printf("[ThreadPoolProc] ERROR: pd->Thread=%p is garbage! Skipping...\n", thread);
			fflush(stdout);
			continue;
		}
		
		// IMPORTANT: Capture these BEFORE thread executes, as thread may be freed during execution
		void *user_param = thread->param;
		THREAD_PROC *user_proc = pd->ThreadProc;
		REF *thread_ref = thread->ref;  // Capture ref for later checking
		EVENT *release_event = thread->release_event;  // Capture event for waiting
		
		// Validate captured pointers
		if (thread_ref == NULL || release_event == NULL)
		{
			printf("[ThreadPoolProc] ERROR: thread_ref=%p or release_event=%p is NULL! Skipping...\n", thread_ref, release_event);
			fflush(stdout);
			continue;
		}
		
		// AddRef to thread_ref and release_event to prevent them from being freed
		AddRef(thread_ref);
		AddRef(release_event->ref);
		...
		// Wait for thread to complete using captured ref (thread struct may be freed!)
		while (true)
		{
			// Use captured thread_ref, not thread->ref
			if (Count(thread_ref->c) <= 1)
			{
				break;
			}

			// Use captured release_event, not thread->release_event
			Wait(release_event, 256);
		}

		ReleaseThread(thread);
		
		// Release our references to prevent memory leak
		Release(thread_ref);
