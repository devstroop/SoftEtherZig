...
void CreateNodeInfo(NODE_INFO *info, CONNECTION *c)
{
	...
	if (product_id != NULL)
	{
		...
		// FIXME: OSGetProductId may return a string literal on some platforms
		// Check if pointer is in heap range before freeing
	...
	{
		printf("[CreateNodeInfo] product_id is NULL, using empty string\n");
		fflush(stdout);
...
bool ClientUploadAuth(CONNECTION *c)
{
	...
	printf("[ClientUploadAuth] Calling OutRpcWinVer...\n");
	fflush(stdout);
	...
	printf("[ClientUploadAuth] HttpClientSend returned: %d\n", ret);
	fflush(stdout);
	if (ret == false)
	{
		c->Err = ERR_DISCONNECTED;
